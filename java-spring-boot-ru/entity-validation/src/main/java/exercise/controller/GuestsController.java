...
public class GuestsController {
    ...
    public GuestDTO addGuest(@Valid @RequestBody GuestCreateDTO createdGuest){
        Guest guest = guestMapper.map(createdGuest);
        guestRepository.save(guest);
        var gst = guestMapper.map(guest);
        return gst;
    }

    //guests и регистрировать нового гостя. Если пользователь вводит невалидные данные, должен вернуться код ответа 400 Bad request.
    // END
}
