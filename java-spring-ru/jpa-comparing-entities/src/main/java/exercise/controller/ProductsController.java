...
public class ProductsController {
    ...
    public ResponseEntity<Product> addProduct(@RequestBody Product data){
        ...
        int match = productRepository.findAll().stream().filter( p -> p.equals(newProduct)).toList().size();

        if (match > 0){
            throw new ResourceAlreadyExistsException("Product with id " + data.getId() + " already exist");
        }
